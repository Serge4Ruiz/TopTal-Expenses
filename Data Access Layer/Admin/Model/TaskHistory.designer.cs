...
namespace TopTal.Expenses.DataAccessLayer.Admin.Model
{
	public partial class TaskHistory : IModel
	{
		...
		public TaskHistory(DataLayer.MongoDB.Admin.TaskHistory taskHistory)
		{
			if (taskHistory == null)
				return;
			Soid = taskHistory.Id.ToString();
		...
		public DataLayer.MongoDB.Admin.TaskHistory ToModel()
		{
			var taskHistory = new DataLayer.MongoDB.Admin.TaskHistory();
			if (!Soid.IsNullOrEmpty())
				taskHistory.ResetId(Soid);
