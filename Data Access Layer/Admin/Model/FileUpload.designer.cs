...
namespace TopTal.Expenses.DataAccessLayer.Admin.Model
{
	public partial class FileUpload : IModel
	{
		...
		public FileUpload(DataLayer.MongoDB.Admin.FileUpload fileUpload)
		{
			if (fileUpload == null)
				return;
			Soid = fileUpload.Id.ToString();
		...
		public DataLayer.MongoDB.Admin.FileUpload ToModel()
		{
			var fileUpload = new DataLayer.MongoDB.Admin.FileUpload();
			if (!Soid.IsNullOrEmpty())
				fileUpload.ResetId(Soid);
