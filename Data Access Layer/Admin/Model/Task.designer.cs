...
namespace TopTal.Expenses.DataAccessLayer.Admin.Model
{
	public partial class Task : IModel
	{
		...
		public Task(DataLayer.MongoDB.Admin.Task task)
		{
			if (task == null)
				return;
			Soid = task.Id.ToString();
			SqlId = task.SqlId;
		...
		public DataLayer.MongoDB.Admin.Task ToModel()
		{
			var task = new DataLayer.MongoDB.Admin.Task();
