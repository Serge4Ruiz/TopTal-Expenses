...
namespace TopTal.Expenses.DataAccessLayer.Analytics.Model
{
	public partial class AccountLogin : IModel
	{
		...
		public AccountLogin(DataLayer.MongoDB.Analytics.AccountLogin accountLogin)
		{
			if (accountLogin == null)
				return;
			Soid = accountLogin.Id.ToString();
		...
		public DataLayer.MongoDB.Analytics.AccountLogin ToModel()
		{
			var accountLogin = new DataLayer.MongoDB.Analytics.AccountLogin();
			if (!Soid.IsNullOrEmpty())
				accountLogin.ResetId(Soid);
